...
- **The impulse is the object's OWN |Δspeed| × mass, and since Phase 31 (M34) the SURFACE that receives it is chosen by APPROACH rather than by manifold impulse** — a contact the object was closing on faster than DAMAGE.property.approachMps, out of whose normal being stopped pushed it, so a floor cannot be credited with a horizontal stop and a manifold is never the only reading; with nothing approached the ranking falls back to the largest manifold, unchanged. The reading itself is still |Δspeed| × mass: a 9 kg box at 4.0 m/s registers 31.3 N·s because 4.7 N·s of it came back as rebound. Consistent, documented, and what every fixture number is measured against.
...
- ~~**A fast, heavy hit can under-read.** A 110 kg fridge at 6 m/s from 0.16 m stops dead (632 N·s of m·Δv) with a first-step manifold of 14 N·s — Rapier resolves the deep first-step penetration by position correction the manifold never reports — and the floor's 18 N·s resting impulse then wins M14's ranking, so the door is NOT forced. From 0.05 m at 4 m/s the same fridge forces it in one step (427.9 N·s). A corner case of thrown appliances; recorded, not fixed.~~ **Closed in Phase 31 (M34) — and the cause recorded above was WRONG.** Re-measured between the solver and the damage system, that step's leaf manifold reads 627.90 N·s, larger than the object's own 598.0 N·s of momentum change. It collapses only when read AFTER the entity loop, because 5.58 m/s breaks the fridge, spawning its fragments re-runs the narrow phase, and the door-frame pass runs after that loop. Nothing to do with position correction. The frame now takes the largest of three readings — the leaf's manifold, the hands' force on a pressed held object, and the object's own momentum change — so no single unreliable one can hide a hit, and the throw takes the door off on the step it lands.
...
**The help line does not name the manifest key (M33).** `#help` is derived from the live binding table and its height is a measured budget — `SETTINGS.textSize.helpMaxLines`, spent by `syncHelpMetrics()` and pinned by m36 S1 — so adding ' M manifest' to the solo control line would spend that budget for a reason unrelated to the card and risk the §21.1 harm the measurement exists to prevent. Discovery today is the pause card's 'The manifest' button and the Settings → Controls row. The honest fix is a shorter control line, which is already an open item here.

**A rebind moves the on-foot key only (M33).** `manifest` is bound in both the FOOT and DRIVE contexts so the list is readable during a leg (§21.2 'during a contract'), but the Controls card lists `LISTED_CONTEXT = foot` alone. Rebinding it therefore moves the on-foot key and leaves the driving one on its default. This is the shipped behaviour for every action bound in both contexts — `interact` has had it since Phase 4 — and m0 B25d/B25e pin the data; it is recorded here because the manifest is the first action a player is likely to rebind and then use from the cab.

**A position change with no event can be one frame late (M33).** The card's per-frame fingerprint covers delivered, room-correct, condition and parts-off; a change of PLACE is caught by the bus (CARGO_STATE, GRIP_ENDED, CONTRACT_PHASE), because `rowWhere()` is the expensive half and running it per frame is what the card exists not to do. An item that slides from the destination apron out of it with nobody touching it — no grip, no load, no phase change — will keep its old state word until the next event. No path in the shipped build does that.

**The house has a lid (measured while building M33, not a regression).** An object parked above about 2.4 m inside the pickup house lands on the roof rather than falling into the room: a lamp set above about 2.4 m in the living room settles on the roof and takes no damage however long it sits there. Any drop test indoors must stay under the ceiling or use the open pad at (40, -40) that m19 established. Recorded so the next suite does not spend a run finding it again.

**Fixed at integration (Phase 31):** the list's height comes from config through one custom property with no literal fallback to go stale, the driving-context binding is documented as the seam it is rather than a rescue for a context nothing enters, and the footer's doors-off segment — shipped and never exercised — now has assertions for both its singular and plural forms.

### M34

- ~~**A fast, heavy hit can under-read.** A 110 kg fridge at 6 m/s from 0.16 m stops dead (632 N·s of m·Δv) with a first-step manifold of 14 N·s — Rapier resolves the deep first-step penetration by position correction the manifold never reports — and the floor's 18 N·s resting impulse then wins M14's ranking, so the door is NOT forced. From 0.05 m at 4 m/s the same fridge forces it in one step (427.9 N·s). A corner case of thrown appliances; recorded, not fixed.~~ **Closed in Phase 31 (M34), and the recorded cause was wrong.** Re-measured: queried between the solver and the damage system, that step's leaf manifold reads **627.90 N·s** — larger than the object's own 598.0 N·s of m·Δv. It collapses only when queried AFTER `damage.step()`'s entity loop, because 5.58 m/s BREAKS a fridge, `breakInto` spawns its fragments, and adding colliders re-runs the narrow phase; the door-frame pass runs after that loop. Nothing to do with position correction. The second reading that should have covered it was unreachable: `_strainFrames` asked M14's ranking about the OBJECT'S OWN collider (`other`, from `contactPairsWith(leaf.collider, …)`), which can never be in its own contact list, so the m·Δv branch returned false for every hit ever taken. Both fixed: the target is the leaf, and the answer comes from the ranking `_attributeProperty` made in the entity loop before any fragmentation. The throw now forces the door on the step it lands, crediting 598.031 N·s and billing chargeForced 140.00 (m41 I1); the 4 m/s case is unchanged to the cent at 427.808.

- **The impulse is the object's OWN |Δspeed| × mass, and the SURFACE is whatever it was travelling into.** A 9 kg box at 4.0 m/s registers 31.3 N·s because 4.7 N·s of it came back as rebound; the vector reading (mass × |Δv along the contact normal|) is the larger of the two — measured 625.7 vs 595.6 N·s on a 110 kg fridge stopped by a wall, 5.1 % apart — and the build deliberately keeps |Δspeed|, because every recorded price is calibrated against it. Since Phase 31 (M34) the surface that receives it is chosen by APPROACH, not by manifold impulse: a contact the object was closing on faster than `DAMAGE.property.approachMps` (0.75 m/s) and whose normal the velocity change points back out of. A floor therefore cannot be credited with a horizontal stop, and a manifold — a proxy that has now been caught reporting a fragmented body's aftermath as the impulse that hit it — is never the only reading. With nothing approached the ranking falls back to M14's largest manifold, unchanged: measured, m22's whole property table (including its merged pair and its §8.3 cap), m25's four packs and m37's split shares are identical either way.

- **A door frame is not bounded by the object's momentum; a wall is.** `_attributeProperty` can never credit a billable surface with more than the object's own m·Δv (asserted, m41 I1d/I2f). `_strainFrames` takes the LARGEST of three readings — the leaf's manifold, the hands' force on a held object pressed against it, and that same m·Δv — and the one that forces a door under a sustained shove is the hands': measured, the couch's own m·Δv sums to **0.00 N·s** across the whole press while the frame accumulates past 400. That is the design (§3.3), not a leak, but it means the momentum ceiling must never be restated as a build-wide invariant, and the hands branch must not be deleted to 'restore' one.
- **The leaf's manifold reading is taken after any fragmentation in the same step.** `_strainFrames` runs after `damage.step()`'s `breakInto` loop, so on a step where an object shatters, the leaf's Σ|contactImpulse| describes where the pieces are, not the blow — measured, **627.90 N·s before that loop and 0.00 after it**, on the throw that forced a door (the same step read from the wall's side, in the throw that holes a wall: 625.79 → 9.55). Harmless now (the frame takes the largest of three readings, and the other two carry a real hit), but the manifold is the LEAST reliable of the three and should not be made load-bearing again. Moving the frame pass ahead of the fragmentation loop is a one-line change nothing currently needs.
- **The approach ranking is a guard, not yet a behaviour change.** On every fixture in the suites it picks the same surface M14's largest-manifold ranking picked — which is why m22's table reproduces to the cent — so no shipped number today depends on it. It exists because the proxy it replaces has been measured lying once already.
- **Fixed at integration (Phase 31):** the assertion the header cites as its proof no longer carries the momentum invariant the milestone retracted; it names the per-step bound it actually checks.
